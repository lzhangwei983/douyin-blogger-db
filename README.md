...
> **使用即同意**：克隆/下载/运行本项目即表示你已阅读并同意 `LICENSE` / `DISCLAIMER.md` / `SECURITY.md`。

- **仅限个人备份与学习研究**：禁止任何商业用途、禁止有偿代下/代运营、禁止将下载内容二次分发或公开传播。
- **遵守平台 ToS 与法律**：自行遵守抖音/快手/B站/YouTube 等平台服务条款及《著作权法》《网络安全法》等；作者不提供也不鼓励绕过付费墙/DRM/风控。
- **自担风险**：作者按“原样”提供软件，不承担因使用产生的版权纠纷、封号、数据丢失等任何责任；如致作者被索赔，你需赔偿。
- **侵权必删**：权利人可提 Issue（标题 `Takedown`）或按 `SECURITY.md` 流程投诉，核实后 48 小时内删除/更正。

**一句话保命**：只备份你有权备份的内容，别商用、别外传、别大规模扒。
